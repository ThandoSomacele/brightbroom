...
<script lang="ts">
  ...
  import { SERVICE_AREAS, type ServiceArea } from '$lib/utils/serviceAreaValidator';
    ...
    try {
      ...
      if (map && circles.length > 0) {
        ...
        SERVICE_AREAS.forEach(area => {
          ...
          bounds.extend(new google.maps.LatLng(area.lat + 0.05, area.lng + 0.05));
          bounds.extend(new google.maps.LatLng(area.lat - 0.05, area.lng - 0.05));
        });
  ...
  function addServiceAreaCircle(area: ServiceArea) {
    ...
    const isSelected = selectedAreaName ? area.name === selectedAreaName : false;
  ...
  // Re-initialize when selectedAreaName changes
...
<div class="service-area-map-container" style="position: relative;">
  {#if error}
    <div class="bg-red-50 p-4 rounded-md text-red-700 dark:bg-red-900/20 dark:text-red-300">
      <p>{error}</p>
    </div>
  {/if}
  ...
  {#if !scriptLoaded}
    <div class="absolute inset-0 flex items-center justify-center bg-white bg-opacity-80 dark:bg-gray-800 dark:bg-opacity-80">
