  ...
  {:else}
    <div class="grid gap-6 md:grid-cols-3">
      {#each cleaners as cleaner (cleaner.id)}
        <div 
          class="cleaner-card border-2 rounded-lg p-4 cursor-pointer transition-all {selectedCleaner?.id === cleaner.id ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-gray-300'}"
          ...
          <div class="flex flex-col items-center">
            <div class="relative mb-3">
              ...
              {#if selectedCleaner?.id === cleaner.id}
                <div class="absolute -bottom-1 -right-1 bg-primary text-white rounded-full p-1" transition:fade>
            ...
            {#if cleaner.rating}
              <div class="flex items-center gap-1 mb-2">
                <span class="text-yellow-500">{getRatingStars(cleaner.rating)}</span>
            ...
            <div class="flex items-center gap-3 mb-3 text-sm">
              {#if cleaner.petCompatibility !== 'NONE'}
                <span title="Pet friendly">{getPetIcon(cleaner.petCompatibility)}</span>
              {/if}
              {#if cleaner.distance}
            ...
            {#if showProfile === cleaner.id}
              <div class="mt-3 pt-3 border-t border-gray-200 w-full" transition:slide>
