...
export async function GET({ url, locals }) {
  try {
    ...
    if (!locals.user) {
      throw redirect(302, `/auth/login?redirectTo=/payment/success?booking_id=${bookingId}`);
    }
    ...
    if (paymentIdParam && paymentIdParam !== paymentData.id) {
      console.warn(`Payment ID mismatch: expected ${paymentData.id}, got ${paymentIdParam}`);
    }
    ...
    try {
      const result = await postPaymentHooks.attemptCleanerAssignment(bookingId);
      
      if (result.success) {
        console.log(`Auto-assigned cleaner ${result.cleanerId} to booking ${bookingId}`);
      } else {
        console.log(`No cleaner auto-assigned to booking ${bookingId}: ${result.message}`);
      }
    } catch (hookError) {
      console.error(`Error in cleaner auto-assignment for booking ${bookingId}:`, hookError);
    }
