...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    if (bookingData.status === "PENDING") {
      ...
      // Create an admin note about the cancellation and email prevention
      await db.insert(adminNote).values({
        id: crypto.randomUUID(),
        bookingId: bookingData.id,
        content:
          "Booking cancelled by customer via payment gateway. Confirmation email will be suppressed.",
    ...
    throw redirect(
      302,
      `/payment/cancel?booking_id=${bookingData.id}&canceled=true`,
    );
  } catch (error) {
