...
<div class="mb-6 flex items-center">
  ...
  <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Cleaner Details</h1>
</div>
      ...
      <div class="p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            on:click={togglePersonalInfoEdit}
          >
            {#if isPersonalInfoEditMode}
                ...
                <div class="grid grid-cols-2 gap-2">
                  {#each Object.keys(availableDays) as day}
                    <label class="flex items-center space-x-2">
                      ...
                      />
                      <span class="text-sm text-gray-700 dark:text-gray-300">{day.charAt(0) + day.slice(1).toLowerCase()}</span>
                    </label>
                  {/each}
                </div>
                <input type="hidden" name="availableDays" value={JSON.stringify(getAvailableDaysArray())} />
              </div>
              ...
              <div>
                <label class="flex items-center space-x-2">
                  <input 
                    ...
                    class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                  />
                  <span class="text-sm text-gray-700 dark:text-gray-300">Available for bookings</span>
                </label>
              </div>
            ...
            {#if cleaner.cleanerProfile}
              ...
              {#if cleaner.cleanerProfile.workAddress}
                <div class="flex items-start">
                  <MapPin class="h-5 w-5 text-gray-500 dark:text-gray-400 mr-2 mt-1" />
                  <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                      Work Location
                    </p>
              ...
              {#if cleaner.cleanerProfile.availableDays && cleaner.cleanerProfile.availableDays.length > 0}
                <div class="flex items-start">
                  <Calendar class="h-5 w-5 text-gray-500 dark:text-gray-400 mr-2 mt-1" />
                  <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                      Available Days
                    </p>
                    <p class="text-gray-900 dark:text-white">
                      {cleaner.cleanerProfile.availableDays.map(day => day.charAt(0) + day.slice(1).toLowerCase()).join(', ')}
                    </p>
                  </div>
                </div>
              {/if}
              ...
              {#if cleaner.cleanerProfile.petCompatibility && cleaner.cleanerProfile.petCompatibility !== 'NONE'}
                <div class="flex items-start">
                  <div class="h-5 w-5 text-gray-500 dark:text-gray-400 mr-2 mt-1">
                  ...
                  <div>
                    ...
                    <p class="text-gray-900 dark:text-white">
                      {cleaner.cleanerProfile.petCompatibility === 'DOGS' ? 'Dogs only' : 
                       cleaner.cleanerProfile.petCompatibility === 'CATS' ? 'Cats only' : 
                       cleaner.cleanerProfile.petCompatibility === 'BOTH' ? 'Dogs and cats' : 'None'}
                    </p>
                  </div>
                </div>
              {/if}
              ...
              {#if cleaner.cleanerProfile.bio}
                <div class="flex items-start">
                  <User class="h-5 w-5 text-gray-500 dark:text-gray-400 mr-2 mt-1" />
                  <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                      Bio
                    </p>
              ...
              <div class="flex items-center mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                  <div class={`h-2.5 w-2.5 rounded-full mr-2 ${cleaner.cleanerProfile.isAvailable ? 'bg-green-500' : 'bg-red-500'}`}></div>
                  <p class="text-sm text-gray-700 dark:text-gray-300">
                    {cleaner.cleanerProfile.isAvailable ? 'Available for bookings' : 'Not available for bookings'}
                  </p>
                </div>
              </div>
            {:else}
        ...
        {#if isSpecializationsEditMode}
          ...
          >
            <div class="space-y-4">
                ...
                <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  <div class="flex items-start">
                    <input 
                      ...
                      checked={selectedSpecializations.get(service.id)?.selected || false}
                    ...
                    <div class="ml-3 flex-1">
                      <label for={`service-${service.id}`} class="text-gray-900 dark:text-white font-medium">
                        {service.name}
                      </label>
                      <p class="text-sm text-gray-500 dark:text-gray-400">
                        {service.description}
                      ...
                      {#if selectedSpecializations.get(service.id)?.selected}
                        <div class="mt-2">
                          <label for={`experience-${service.id}`} class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Experience (months)
                          </label>
                          <input
                            type="number"
                            id={`experience-${service.id}`}
                            value={selectedSpecializations.get(service.id)?.experience || 0}
                            on:input={(e) => updateServiceExperience(service.id, parseInt(e.target.value))}
                            min="0"
              ...
              <input type="hidden" name="specializations" value={getSelectedSpecializations()} />
          ...
          {#if cleaner.specializations && cleaner.specializations.length > 0}
            <div class="space-y-3">
              {#each cleaner.specializations as specialization}
                <div class="flex items-start">
                  <Briefcase class="h-5 w-5 text-gray-500 dark:text-gray-400 mr-3" />
                  <div>
