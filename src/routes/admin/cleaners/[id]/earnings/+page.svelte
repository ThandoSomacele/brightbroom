...
<svelte:head>
  <title>Cleaner Earnings | {cleaner.firstName} {cleaner.lastName} | BrightBroom Admin</title>
</svelte:head>
...
<div class="mb-6 flex items-center">
  ...
  <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
    Earnings for {cleaner.firstName} {cleaner.lastName}
  </h1>
</div>

<!-- Success/Error messages -->
{#if form?.success}
  <div class="mb-6 rounded-md bg-green-50 p-4 text-green-800 dark:bg-green-900/20 dark:text-green-300">
    <p>{form.message}</p>
  </div>
{:else if form?.error}
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <th scope="col" class="w-10 px-3 py-3 text-left">
                    <div class="flex items-center">
                      <input
                        ...
                        checked={selectedPayments.size === pendingPayouts.length && pendingPayouts.length > 0}
                        on:change={toggleAllPayments}
                      />
                    </div>
                  </th>
                  <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">
                    Date
                  </th>
                  <th scope="col" class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">
                    Booking
                  </th>
                  <th scope="col" class="px-3 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">
                    Amount
                  </th>
                </tr>
              </thead>
              ...
              <tfoot class="bg-gray-50 dark:bg-gray-700">
                <tr>
                  <td colspan="3" class="px-3 py-2 text-right text-sm font-medium text-gray-700 dark:text-gray-300">
                    Total Selected:
                  </td>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th scope="col" class="px-3 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">
                    Status
                  </th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                {#each displayedPayments as payment}
                  <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white text-right">
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
      ...
      <p class="text-gray-700 dark:text-gray-300 mb-4">
        Are you sure you want to process a payout of <span class="font-bold text-primary">{formatCurrency(selectedPayoutAmount)}</span> for {selectedPayments.size} payment{selectedPayments.size !== 1 ? 's' : ''}?
      </p>
      
      <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-md mb-4">
        <p class="text-yellow-800 dark:text-yellow-300 text-sm">
          This action will mark these payments as paid to the cleaner and cannot be undone. Ensure you have processed the actual bank transfer before confirming.
        </p>
      </div>
