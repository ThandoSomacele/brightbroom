...
<script lang="ts">
  ...
  const { cleaners, recentPayouts, totals } = data;
  ...
  // Filter cleaners by payout method
  $: eftCleaners = cleaners.filter(
    (c) => c.payoutMethod === "EFT" && Number(c.pendingPayout) > 0
  );
  $: instantMoneyCleaners = cleaners.filter(
    (c) => c.payoutMethod !== "EFT" && Number(c.pendingPayout) > 0
  );

  // Calculate selected totals
  $: selectedEftTotal = eftCleaners
    .filter((c) => selectedEftCleaners.has(c.id))
    .reduce((sum, c) => sum + Number(c.pendingPayout), 0);

  $: selectedInstantMoneyTotal = instantMoneyCleaners
    .filter((c) => selectedInstantMoneyCleaners.has(c.id))
    .reduce((sum, c) => sum + Number(c.pendingPayout), 0);
  ...
  function selectAllInstantMoney() {
  ...
  // Generate EFT CSV for Standard Bank Business Online
  function generateEftCsv() {
    const selectedCleaners = eftCleaners.filter((c) =>
      selectedEftCleaners.has(c.id)
    );

    if (selectedCleaners.length === 0) {
      alert("Please select at least one cleaner for EFT payout");
      return;
    }

    // Standard Bank Business Online EFT CSV format
    // Beneficiary Name, Bank Name, Branch Code, Account Number, Account Type, Amount, Reference
    const headers = [
      "Beneficiary Name",
      "Bank Name",
      "Branch Code",
    ...
    const rows = selectedCleaners.map((cleaner) => [
      cleaner.bankAccountHolder || `${cleaner.firstName} ${cleaner.lastName}`,
      cleaner.bankName || "",
      cleaner.bankBranchCode || "",
      cleaner.bankAccountNumber || "",
      cleaner.bankAccountType === "SAVINGS"
        ? "Savings"
        : cleaner.bankAccountType === "CHEQUE"
          ? "Current"
          : "Transmission",
      Number(cleaner.pendingPayout).toFixed(2),
      `BB-${cleaner.firstName?.charAt(0)}${cleaner.lastName?.charAt(0)}-${new Date().toISOString().slice(0, 10)}`,
    ]);
    ...
    downloadCsv(csvContent, `brightbroom-eft-payouts-${new Date().toISOString().slice(0, 10)}.csv`);
  }

  // Generate Instant Money CSV
  function generateInstantMoneyCsv() {
    const selectedCleaners = instantMoneyCleaners.filter((c) =>
      selectedInstantMoneyCleaners.has(c.id)
    );

    if (selectedCleaners.length === 0) {
      alert("Please select at least one cleaner for Instant Money payout");
      return;
    }

    // Standard Bank Instant Money CSV format
    // Cell Number, Amount, Reference, First Name, Last Name
    const headers = ["Cell Number", "Amount", "Reference", "First Name", "Last Name"];
    ...
    downloadCsv(csvContent, `brightbroom-instant-money-payouts-${new Date().toISOString().slice(0, 10)}.csv`);
  }
    ...
    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center justify-between">
        ...
        <Button
          variant="primary"
          size="sm"
          on:click={generateInstantMoneyCsv}
        ...
        <div class="mb-3">
          <label class="flex items-center space-x-2">
            <input
              ...
              on:change={selectAllInstantMoney}
              class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300">
