...
<script lang="ts">
  ...
  import { Calendar, Clock, MapPin, Repeat, Wallet } from "lucide-svelte";
  ...
  let confirmingAction: { action: string; subscriptionId: string } | null =
    null;
  ...
  <div class="mx-auto max-w-6xl">
        ...
        <div class="flex">
          ...
          <div class="ml-3 flex-1">
            <p class="text-sm text-blue-700 dark:text-blue-200">
              Your subscription payment is being processed. This may take a few
              moments. The subscription will become active once the payment is
              confirmed.
            </p>
          </div>
        </div>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
      <div class="flex flex-wrap gap-2">
        <span
          class="text-sm font-medium text-gray-700 dark:text-gray-300 mr-2 py-2"
          >Status:</span
        >
        <button
            ...
            <div class="flex flex-wrap justify-between gap-4">
              ...
              <div class="flex-1">
                <div class="mb-4 flex items-center">
                  <span
                    class={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${getStatusBadgeClass(subscription.status)}`}
                  >
                    {subscription.status}
                  </span>
                </div>
                  ...
                  <p class="flex items-center text-gray-600 dark:text-gray-300">
                    ...
                    {#if subscription.preferredDays && subscription.preferredDays.length > 0}
                      <span class="ml-2 text-sm"
                        >({formatDays(subscription.preferredDays)})</span
                      >
                    {/if}
                  </p>
                      ...
                      <div class="space-y-1">
                        {#each subscription.payments.slice(0, 3) as payment}
                          ...
                          >
                            ...
                            <span
                              class={`font-medium ${payment.status === "COMPLETED" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                            >
                              R{typeof payment.amount === "number"
                ...
                <div class="mt-4 flex flex-wrap gap-2 justify-end">
                  {#if subscription.status === "ACTIVE" && subscription.payFastToken}
                    <Button
                      variant="outline"
                      on:click={() =>
                        showConfirmation("pause", subscription.id)}
                    >
                      Pause
                    </Button>
                  {/if}
                  ...
                  {#if subscription.status === "PAUSED" && subscription.payFastToken}
                    <Button
                      variant="outline"
                      on:click={() =>
                        showConfirmation("resume", subscription.id)}
                    >
                      Resume
                    </Button>
                  {/if}
                  ...
                  {#if (subscription.status === "ACTIVE" || subscription.status === "PAUSED") && subscription.payFastToken}
                    <Button
                      ...
                      on:click={() =>
                        showConfirmation("cancel", subscription.id)}
                    >
                      Cancel
                    </Button>
                  {/if}
        ...
        >
          ...
          {#if filterStatus !== "all"}
            <p class="mb-4 text-gray-600 dark:text-gray-400">
              No subscriptions found for the selected filter. Try adjusting your
              filter or create a new subscription.
            </p>
            <div class="flex flex-wrap gap-2 justify-center">
              <Button variant="outline" on:click={() => (filterStatus = "all")}>
                Clear filter
              </Button>
              <Button variant="secondary" href="/book">New Subscription</Button>
            </div>
          {:else}
            <p class="mb-4 text-gray-600 dark:text-gray-400">
              You don't have any active subscriptions. Subscribe now for
              recurring cleaning services!
            </p>
            <Button variant="secondary" href="/book">Subscribe Now</Button>
          {/if}
        </div>
      {/if}
    </div>
  ...
  >
    <div class="max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
      <h3 class="mb-4 text-xl font-semibold text-gray-900 dark:text-white">
        Confirm {confirmingAction.action.charAt(0).toUpperCase() +
          confirmingAction.action.slice(1)}
      </h3>

      <p class="mb-6 text-gray-600 dark:text-gray-300">
        {#if confirmingAction.action === "pause"}
          Are you sure you want to pause this subscription? You can resume it
          anytime.
        {:else if confirmingAction.action === "resume"}
          Are you sure you want to resume this subscription? Billing will
      ...
      <div class="flex gap-3 justify-end">
        <Button variant="outline" on:click={cancelConfirmation}>Cancel</Button>

        <form method="POST" action={`?/${confirmingAction.action}`} use:enhance>
          <input
            type="hidden"
            name="subscriptionId"
            value={confirmingAction.subscriptionId}
          />
          <input
            type="hidden"
            name="token"
            value={subscriptions.find(
              (s) => s.id === confirmingAction.subscriptionId,
            )?.payFastToken || ""}
          />
