...
{#await data.streamed.earningsData}
  <CustomerSkeleton variant="earnings" />
{:then earningsData}
  {@const summary = earningsData.summary}
  {@const payoutDates = earningsData.payoutDates}
  {@const completedBookings = earningsData.completedBookings}
          ...
          {#each monthlyEarnings as { month, year, earnings }, i}
            <div class="flex-1 flex flex-col items-center">
              <div
                ...
                style="height: {Math.max(10, (earnings / Math.max(...monthlyEarnings.map(m => m.earnings), 1)) * 80)}%"
              ></div>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                      {formatCurrency(Number(booking.price) * 0.75)}
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    {#if booking.isPaid}
