...
async function updateAddressCoordinates() {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Error: The lat/lng columns don't exist in the address table!",
      );
    ...
    for (const addr of addressesToUpdate) {
      try {
        ...
        if (coordinates) {
          ...
          console.log(
            `✅ Updated address ${addr.id} with coordinates: ${coordinates.lat}, ${coordinates.lng}`,
          );
        } else {
          console.warn(
            `⚠️ Failed to geocode address ${addr.id}: ${fullAddress}`,
          );
        }
